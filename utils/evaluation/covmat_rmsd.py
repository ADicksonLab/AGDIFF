...
class CovMatEvaluator(object):
    ...
    def __call__(self, packed_data_list, start_idx=0):
        ...
        for idx, confusion_mat in enumerate(tqdm(self.pool.imap(func, filtered_data_list), total=len(filtered_data_list))):
            ...
            num_gen = filtered_data_list[idx]['pos_gen'].shape[0]
            num_ref = filtered_data_list[idx]['pos_ref'].shape[0]
            smiles = filtered_data_list[idx]['smiles']
