...
class PropertyCalculator(object):
    ...
    def __call__(self, data, num_confs=50):
        ...
        for idx in tqdm(conf_idx):
            ...
            self.pk.mol = mol
            try:
                energy, homo, lumo, dipo = (
                    self.pk.energy(),
                    self.pk.HOMO,
                    self.pk.LUMO,
                    self.pk.dipolemoment[-1],
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, default="./data/GEOM/QM9/qm9_property.pkl"
    )
    ...
    prop_cal = PropertyCalculator(
        threads=args.threads, memory=args.memory, seed=args.seed
    )

    cache_ref_fn = os.path.join(
        os.path.dirname(args.dataset), os.path.basename(args.dataset)[:-4] + "_prop.pkl"
    )
