...
def get_angle(pos, angle_index):
    ...
    v1 = pos[n1] - pos[ctr]
    v2 = pos[n2] - pos[ctr]
    inner_prod = torch.sum(v1 * v2, dim=-1, keepdim=True)
    length_prod = torch.norm(v1, dim=-1, keepdim=True) * torch.norm(v2, dim=-1, keepdim=True)
    cos_angle = inner_prod / length_prod
    cos_angle = torch.clamp(cos_angle, -1, 1)  # Clamp to avoid domain errors in acos
