...
class DistanceWeightingNetwork(nn.Module):
    ...
    def forward(self, edge_length):
        edge_length = edge_length.unsqueeze(-1)  # Ensure edge_length is properly shaped for linear layers
        x = self.activation(self.layer1(edge_length))
        weights = torch.sigmoid(self.layer2(x))  # Use sigmoid to ensure output weights are between 0 and 1
        return weights.squeeze(-1)  # Remove the last dimension to match the expected shape
...
class CFConv(MessagePassing):
    def __init__(self, in_channels, out_channels, num_filters, nn, cutoff, smooth, hidden_dim=32):
...
class InteractionBlock(torch.nn.Module):
    def __init__(self, hidden_channels, num_gaussians, num_filters, cutoff, smooth):
        ...
        self.conv1 = CFConv(hidden_channels, hidden_channels, num_filters, mlp1, cutoff, smooth)
        self.conv2 = CFConv(hidden_channels, hidden_channels, num_filters // 2, mlp2, cutoff, smooth)

        self.act = ShiftedSoftplus()

        self.lin = Linear(256, hidden_channels)  # Adjusted to match the actual concatenated size
...
class SchNetEncoder(Module):
    def __init__(self, hidden_channels=128, num_filters=128, num_interactions=6, edge_channels=100, cutoff=10.0, smooth=False):
        ...
        for _ in range(num_interactions):
            self.interactions.append(InteractionBlock(hidden_channels, edge_channels, num_filters, cutoff, smooth))
    ...
    def forward(self, z, edge_index, edge_length, edge_attr, embed_node:bool=True):
        ...
        for interaction, scale in zip(self.interactions, self.scaling_modules):
            ...
            scaled_interaction_output = scale(interaction_output.unsqueeze(-1)).squeeze(-1)  # Assuming h is of shape (batch_size, num_channels)
