...
def rdmol_to_data(mol: Mol, pdb_path: str, smiles=None):
    ...
    pos_ref = torch.tensor(mdj.load(pdb_path).xyz).squeeze(
        0
    )  # Make sure the PDB file has the right reference coordinates in case you wanna evaluate samples based on the COV and MAT metrics.
    data = Data(
        atom_type=z,
        pos=pos,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_traj",
        action="store_true",
        default=False,  # Make sure to dont save the whole trajectory in case you wanna evaluate the samples based on the COV and MAT metrics, or you can save the trajs in output with a different variable rather than pos_gen.
